...
for i, g in df_train.groupby('Country_Region'):
    ...
    for i, r in df_tmp.iterrows():
        ...
        df_submission.loc[i, 'ConfirmedCases'] = max(predict_next_values(p_confirmed, [r['Days']])[0], 0)

        df_submission.loc[i, 'Fatalities'] = max(predict_next_values(p_fatal, [r['Days']])[0], 0)
