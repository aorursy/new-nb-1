...
#inplace digunakan untuk masukin ke testnya. Atau bisa juga gapake inplace, tapi test = test.rename
...
def split_train_test(dataset,end_of_training_date):
    ...
    end_date = pd.to_datetime(end_of_training_date, format="%Y-%m-%d")

    training_data = dataset.loc['20170105':end_date]

    testing_data = dataset.loc[end_date + timedelta(days=1):]

    return training_data,testing_data



def split_label_and_predictor(train_test):

    x_data = train_test.iloc[:,1:]

    y_data = train_test.iloc[:,0]

    return x_data,y_data
# Pisahkan kolom yang ingin diprediksi (biasa disebut label menjadi y), dan variabel lain menjadi x

train_data,test_data=split_train_test(daily_online_revenue,"2017-11-30")

x_train,y_train=split_label_and_predictor(train_data)

x_test,y_test=split_label_and_predictor(test_data)

#x_train.drop(["Date"], inplace=True)

#x_test.drop(["Date"], inplace=True)
...
for i in range(n_iteration):
    ...
    _, testing_data = split_train_test(daily_online_revenue,"2017-11-30")

    x_test, _ = split_label_and_predictor(testing_data)

result
y_test=y_test.reset_index(name='Revenue')
