...
def create_placeholders(height=128, width=128, channels=4, n_targets=28):
 ...
 #   Best explored architecture so far
...
def multitask_heads_weighted_hinge(mlt,weights,labels):
    ...
    for i,w in enumerate(weights):
        ...
        loss      += tf.reduce_mean(weighting*tf.maximum(0., 1 - labels[:,i]*mlt[i]))
    return predict_proba, loss
...
def multitask_heads_weighted_hinge_squared(mlt,weights,labels):
    ...
    for i,w in enumerate(weights):
        ...
        task_label       = tf.expand_dims(tf.gather(labels_transformed,indices=i,axis=1),axis=1)
        weighting        = tf.pow(tf.constant(w,dtype=tf.float32),task_label)
        loss             = tf.reduce_mean(weighting*tf.square(tf.maximum(0,1-labels[:,i]*mlt[i])))
    return predict_proba, loss


def _predict_probabilities(x_batch,X,sess,predict_proba):
...
with tf.device('/device:GPU:0'):
    ...
    if LOSS_TO_USE == 'Focal':
        predict_proba, loss    = multitask_heads_focal(mlt_logits,balanced_weights,Y, gamma=3)
    ...
    predict_probabilities  = partial(_predict_probabilities,X=X,sess=sess,predict_proba=predict_proba)
    ...
    for i,(batch_images,batch_labels) in enumerate(train_gen): 
        if AUGMENT:
            augmenter = iaa.Sequential([ iaa.OneOf([ iaa.Affine(rotate=0), iaa.Affine(rotate=90), iaa.Affine(rotate=180),
                                                 iaa.Affine(rotate=270),     iaa.Fliplr(0.5), iaa.Flipud(0.5)])])
        ...
        _, loss_val = sess.run([opt,loss], feed_dict={X:batch_images,Y:batch_labels,is_train:False})
        if i%100==0:
...
best_f1 =  evaluate_given_thresholds(probs,best_threshold,labels_val)
