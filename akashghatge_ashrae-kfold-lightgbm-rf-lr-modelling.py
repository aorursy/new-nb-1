...
model_rf = RandomForestRegressor(n_estimators=60,
...
def get_evaluations(model):
    ...
    print('Test_rmse: ',RMSE(y_test,preds))

    

get_evaluations(model_rf)
...
alpha = [1e-4, 1e-3,1e-2, 1, 5, 10, 20]
...
df
pd.DataFrame(ridge_regressor.cv_results_)[['params','mean_test_score']]
get_evaluations(ridge_regressor)
from sklearn.linear_model import Lasso
