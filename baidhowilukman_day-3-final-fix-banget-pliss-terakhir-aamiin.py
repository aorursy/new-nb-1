...
def preprocess (dataset):
    ...
    processed_dataset['r-1']=dataset['Revenue'].shift(periods=1)

    processed_dataset['r-2']=dataset['Revenue'].shift(periods=2)

    processed_dataset['r-3']=dataset['Revenue'].shift(periods=3)

    processed_dataset['r-4']=dataset['Revenue'].shift(periods=4)

    processed_dataset['r-5']=dataset['Revenue'].shift(periods=5)

    processed_dataset['r-6']=dataset['Revenue'].shift(periods=6)

    processed_dataset['r-7']=dataset['Revenue'].shift(periods=7)

    processed_dataset['r-8']=dataset['Revenue'].shift(periods=8)

    processed_dataset['r-9']=dataset['Revenue'].shift(periods=9)

    processed_dataset['r-10']=dataset['Revenue'].shift(periods=10)

    processed_dataset['r-11']=dataset['Revenue'].shift(periods=11)

    processed_dataset['r-12']=dataset['Revenue'].shift(periods=12)

    processed_dataset['r-13']=dataset['Revenue'].shift(periods=13)

    processed_dataset['r-14']=dataset['Revenue'].shift(periods=14)

    processed_dataset['r-15']=dataset['Revenue'].shift(periods=15)

        

    processed_dataset = processed_dataset[pd.notnull(processed_dataset['r-15'])]
...
#     end_of_training_date = pd.datetime(end_of_training_date)

    training_data = dataset[(dataset.index < end_of_training_date)]

    testing_data = dataset[(dataset.index >= end_of_training_date)]
...
train_data , test_data = split_train_test(daily_online_revenue,end_of_training_date)
...
def split_label_and_predictor(train_or_test_data):
    ...
    x_data = train_or_test_data.drop(['Revenue'], axis=1)
...
x_train,y_train = split_label_and_predictor(train_data)
