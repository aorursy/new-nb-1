...
dtrain = xgb.DMatrix(ds_train[a],label=ds_train['outcome'])

#print (dtrain)



dtest = xgb.DMatrix(ds_test[a])
...
#gbm.predict(xgb.DMatrix(X_valid[features]), ntree_limit=gbm.best_iteration+1)
dtrain = xgb.DMatrix(ds_train[a],label=ds_train['outcome'])
...
boost = xgb.train(param, dtrain, num_round, early_stopping_rounds=early_stopping_rounds)
