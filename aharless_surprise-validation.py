...
print('RMSE GradientBoostingRegressor: ', RMSLE(np.log1p(actuals['visitors'].values), preds1,

                                                sample_weight=weights.values))

print('RMSE KNeighborsRegressor: ', RMSLE(np.log1p(actuals['visitors'].values), preds2,
...
print('RMSE NeuralNetwork: ', RMSLE(np.log1p(actuals['visitors'].values), preds4,
...
print('RMSE GradientBoostingRegressor: ', RMSLE(np.log1p(actuals['visitors'].values), 

                                                preds1, sample_weight=public_weights) )

print('RMSE KNeighborsRegressor: ', RMSLE(np.log1p(actuals['visitors'].values), 

                                          preds2, sample_weight=public_weights) )

print('RMSE XGBRegressor: ', RMSLE(np.log1p(actuals['visitors'].values), 

                                   preds3, sample_weight=public_weights) )

print('RMSE NeuralNetwork: ', RMSLE(np.log1p(actuals['visitors'].values), 

                                    preds4, sample_weight=public_weights) )
...
print('RMSE GradientBoostingRegressor: ', RMSLE(np.log1p(actuals['visitors'].values), 
                                                ...
                                                preds1, sample_weight=private_weights) )

print('RMSE KNeighborsRegressor: ', RMSLE(np.log1p(actuals['visitors'].values), 

                                          preds2, sample_weight=private_weights) )

print('RMSE XGBRegressor: ', RMSLE(np.log1p(actuals['visitors'].values), 

                                   preds3, sample_weight=private_weights) )

print('RMSE NeuralNetwork: ', RMSLE(np.log1p(actuals['visitors'].values), 

                                    preds4, sample_weight=private_weights) )



test['visitors'] = 0.2*preds1+0.15*preds2+0.35*preds3+0.3*preds4
...
the_visit_date = pd.to_datetime(air_visit_data['visit_date'])

air_visit_data = air_visit_data[the_visit_date < pd.to_datetime(VALIDATION_START)]
...
sample_submission['calendar_date'] -= pd.Timedelta(VALIDATION_DELTA)

sample_submission['calendar_date'] = sample_submission['calendar_date'].dt.strftime('%Y-%m-%d')



sample_submission['air_store_id'] = sample_submission.id.map(lambda x: '_'.join(x.split('_')[:-1]))

sample_submission['id'] = sample_submission['air_store_id'] + '_' + sample_submission['calendar_date']
...
sub_merge[['id', 
           ...
           'visitors']].merge(actuals, 

                              on='id', 

                              suffixes=['_pred', '_actual']).to_csv('validation.csv', index=False)

print("Done")
sub_merge.head()
print('RMSE: ', RMSLE( np.log1p(actuals['visitors'].values), 

                       np.log1p(sub_merge['visitors'].values),

                       sample_weight=weights.values )  )
print('Public RMSE: ', RMSLE( np.log1p(actuals['visitors'].values), 

                              np.log1p(sub_merge['visitors'].values),

                              sample_weight=public_weights ))
