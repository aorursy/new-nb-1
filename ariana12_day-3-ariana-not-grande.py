...
df_rev["Revenue1"] = daily_online_revenue['Revenue'].shift(1,fill_value=0 )

df_rev["Revenue2"] = daily_online_revenue['Revenue'].shift(2,fill_value=0 )

df_rev["Revenue3"] = daily_online_revenue['Revenue'].shift(3,fill_value=0 )

df_rev["Revenue4"] = daily_online_revenue['Revenue'].shift(4,fill_value=0 )

df_rev["Revenue5"] = daily_online_revenue['Revenue'].shift(5,fill_value=0 )
...
train_data = df_rev[:end_of_training_date]

test_data  = df_rev[end_of_training_date:].head(14)
train_data
...
def preprocess (dataset):
    ...
    processed_dataset['Revenue1'] = dataset['Revenue'].shift(1,fill_value=0 )

    processed_dataset['Revenue2'] = dataset['Revenue'].shift(2,fill_value=0 )

    processed_dataset['Revenue3'] = dataset['Revenue'].shift(3,fill_value=0 )

    processed_dataset['Revenue4'] = dataset['Revenue'].shift(4,fill_value=0 )

    processed_dataset['Revenue5'] = dataset['Revenue'].shift(5,fill_value=0 )
...
def split_train_test(dataset, end_of_training_date):
    ...
    training_data = dataset[:end_of_training_date]

    testing_data  = dataset[end_of_training_date:].head(14)
...
for i in range(n_iteration):
    ...
    df_rev = preprocess(df_rev2).set_index("Date")

    _, testing_data = split_train_test(df_rev,end_of_training_date)
