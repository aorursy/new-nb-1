...
print("Partial 'Private' mse:     ", mean_squared_error(
    ...
    sample_weight=weights) )
pd.DataFrame(np.array(val_pred).squeeze(axis=2).transpose()).to_csv('nn_val_first_half.csv', 
...
submission.to_csv('nn_sub_first_half.csv', float_format='%.5f', index=None)